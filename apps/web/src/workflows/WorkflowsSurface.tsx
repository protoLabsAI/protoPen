...
export function WorkflowsSurface({ onError }: { onError: (message: string) => void }) {
  ...
  const current = useMemo(() => workflows?.find((w) => w.name === selected) ?? null, [workflows, selected]);
                ...
                {current.inputs.map((inp) => (
                  <label className="field" key={inp.name}>
                    ...
                    <input
                      ...
                      placeholder={
                        inp.default != null ? `default: ${String(inp.default)}` : inp.required ? "required" : "optional"
                      }
                    />
                  </label>
                ))}
