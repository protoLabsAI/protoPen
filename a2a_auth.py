...
def configure(*, bearer_token: str | None, api_key: str, allowed_origins_raw: str) -> None:
    ...
    Args:
        bearer_token: the caller's authoritative token (from YAML ``auth.token``,
            with the caller applying any ``A2A_AUTH_TOKEN`` env fallback). An
            empty string means open mode — only ``None`` falls back to the env,
            so a caller that passes ``""`` (e.g. an API-key-only agent) can't
            have bearer auth silently re-enabled behind the advertised card.
...
class A2AAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # set AND the request actually carries an Origin header. Server-to-server
        # A2A callers (the hub, the scheduler loopback) send no Origin and must
        # not be rejected; only cross-origin browser requests carry one.
