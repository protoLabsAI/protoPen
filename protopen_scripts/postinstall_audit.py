...
DANGEROUS_PATTERNS: list[tuple[re.Pattern, str, str]] = [
    ...
    (
        re.compile(r"base64\s+--decode|-d\b.*base64", re.IGNORECASE),
        "critical",
...
def main() -> None:
    ...
    try:
        ...
        package_files = [p for p in all_packages if "node_modules" not in p.split(os.sep)[:-1]]

        # Also include node_modules package.json for completeness (limit depth)
        nm_packages = [
            p
            for p in all_packages
            if "/node_modules/" in p and p.count("/node_modules/") == 1  # only top-level node_modules
        ]
