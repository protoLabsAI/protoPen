...
AMSI_SIGNATURES: list[tuple[re.Pattern, str, str]] = [
    ...
    (
        re.compile(r"Net\.WebClient|DownloadString|DownloadFile", re.IGNORECASE),
        "ps_downloader",
        "PowerShell web downloader",
    ),
    (
        re.compile(r"AmsiScanBuffer|AmsiInitialize|amsi\.dll", re.IGNORECASE),
        "amsi_bypass",
        "AMSI bypass attempt targeting AMSI itself",
    ),
    (
        re.compile(r"\[Ref\]\.Assembly\.Load|Assembly\.Load\(", re.IGNORECASE),
        "ps_assembly_load",
        "PowerShell assembly loading (in-memory execution)",
    ),
    (
        re.compile(r"VirtualAlloc|WriteProcessMemory|CreateThread", re.IGNORECASE),
        "mem_injection",
        "Memory injection API calls",
    ),
    (
        re.compile(r"base64_decode|FromBase64String", re.IGNORECASE),
        "base64_decode",
    ...
    (
        re.compile(r"powersploit|nishang|powercat", re.IGNORECASE),
        "offensive_ps_framework",
        "Offensive PowerShell framework",
    ),
    (
        re.compile(r"-Enc(?:oded)?Command\s+[A-Za-z0-9+/=]{20,}", re.IGNORECASE),
        "ps_encoded_cmd",
        "PowerShell -EncodedCommand (obfuscated)",
    ),
    (
        re.compile(r"\$env:COMPUTERNAME|\$env:USERNAME|\$env:TEMP", re.IGNORECASE),
        "ps_env_enum",
        "PowerShell environment enumeration",
    ),
    ...
    (
        re.compile(r"whoami\s*/priv|SeDebugPrivilege|SeImpersonatePrivilege", re.IGNORECASE),
        "priv_check",
    ...
    (
        re.compile(r"Add-MpPreference\s+-ExclusionPath", re.IGNORECASE),
        "av_exclusion",
        "Windows Defender exclusion addition",
    ),
    (
    ...
    (
        re.compile(rb"\xfc\xe8\x89\x00\x00\x00|\xfc\xe8\x82\x00\x00\x00".decode("latin-1"), re.DOTALL),
        "shellcode_stub",
        "Common shellcode stager pattern",
    ),
...
def main() -> None:
    ...
    try:
                    ...
                    signatures_found.append(
                        {
                            ...
                            "severity": "high"
                            if sig_name in ("amsi_bypass", "mimikatz", "mem_injection", "c2_framework")
                            else "medium",
                        }
                    )
            except Exception:
        ...
        overall_severity = (
            "info" if sig_count == 0 else ("critical" if sig_count >= 3 else "medium" if sig_count >= 1 else "info")
        )
        ...
        result["findings"].append(
            {
                ...
                if is_pe
                else ("PowerShell" if ext in POWERSHELL_EXTENSIONS else "text/other"),
                "is_binary": is_binary,
