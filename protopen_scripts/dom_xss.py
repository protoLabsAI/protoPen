...
SINK_PATTERNS: list[tuple[str, re.Pattern, str]] = [
    ...
    (
        "setTimeout",
        re.compile(r"""setTimeout\s*\((?!function|=>|\()([^,)]+)[,)]"""),
        "setTimeout with string argument (code execution)",
    ),
    (
        "setInterval",
        re.compile(r"""setInterval\s*\((?!function|=>|\()([^,)]+)[,)]"""),
        "setInterval with string argument (code execution)",
    ),
    (
    ...
    (
        "insertAdjacentHTML",
        re.compile(r"""\.insertAdjacentHTML\s*\(\s*['"][^'"]+['"]\s*,\s*(?!['"`])([^)]+)\)"""),
        "insertAdjacentHTML with variable content",
    ),
    (
        "location.href",
    ...
    (
        ...
        "location.replace with variable argument",
    ),
    (
        "src assignment",
