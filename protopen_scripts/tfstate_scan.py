...
SECRET_PATTERNS: list[tuple[re.Pattern, str, str, str]] = [
    (
        re.compile(r"-----BEGIN\s+(?:RSA|EC|DSA|OPENSSH|PGP|PRIVATE)\s+PRIVATE\s+KEY-----"),
        "private_key",
        "critical",
        "Private key material",
    ),
    (
        re.compile(r'"(?:password|passwd|pass)"\s*:\s*"([^"]{4,})"', re.IGNORECASE),
        "password",
        "critical",
        "Password in Terraform state",
    ),
    (
        re.compile(r'"(?:secret_key|secret_access_key|aws_secret)"\s*:\s*"([A-Za-z0-9/+]{20,})"', re.IGNORECASE),
        "aws_secret_key",
        "critical",
        "AWS secret access key",
    ),
    (
        re.compile(r'"(?:access_key|aws_access_key_id)"\s*:\s*"(AKI[A-Z0-9]{16,})"', re.IGNORECASE),
        "aws_access_key",
        "critical",
        "AWS access key ID",
    ),
    (
        re.compile(r'"(?:api_key|apikey|api_token)"\s*:\s*"([A-Za-z0-9_\-]{20,})"', re.IGNORECASE),
        "api_key",
        "high",
        "API key",
    ),
    (
        re.compile(r'"(?:token|auth_token|access_token|refresh_token)"\s*:\s*"([A-Za-z0-9_\-\.]{20,})"', re.IGNORECASE),
        "token",
        "high",
        "Auth token",
    ),
    (
        re.compile(r'"(?:private_key|rsa_private_key)"\s*:\s*"(-----BEGIN)', re.IGNORECASE),
        "private_key",
        "critical",
        "Private key reference",
    ),
    (
        re.compile(r'"connection_string"\s*:\s*"([^"]{10,})"', re.IGNORECASE),
        "connection_string",
        "high",
        "Database connection string",
    ),
    (
        re.compile(r'"(?:database_url|db_url|db_password)"\s*:\s*"([^"]{4,})"', re.IGNORECASE),
        "database_cred",
        "critical",
        "Database credential",
    ),
    ...
    (
        re.compile(r'"(?:client_secret|oauth_secret)"\s*:\s*"([A-Za-z0-9_\-]{8,})"', re.IGNORECASE),
        "oauth_secret",
        "high",
        "OAuth client secret",
    ),
    ...
    (
        re.compile(
            r'"(?:private_dns|private_ip)"\s*:\s*"(10\.|172\.1[6-9]\.|172\.2[0-9]\.|172\.3[01]\.|192\.168\.)',
            re.IGNORECASE,
        ),
        "private_ip",
        "medium",
        "Private IP address exposed in state",
    ),
    (
        re.compile(r'"(?:account_id|aws_account_id)"\s*:\s*"(\d{12})"', re.IGNORECASE),
        "aws_account_id",
        "medium",
        "AWS Account ID",
    ),
