...
SSR_PATTERNS = [
    ...
    (
        re.compile(r"<script[^>]*>\s*window\.__REDUX_STATE__\s*=\s*({.*?})\s*;?\s*</script>", re.DOTALL),
        "__REDUX_STATE__",
    ),
    (
        re.compile(r"<script[^>]*>\s*window\.__INITIAL_STATE__\s*=\s*({.*?})\s*;?\s*</script>", re.DOTALL),
        "__INITIAL_STATE__",
    ),
    ...
    (
        re.compile(r'<script[^>]*type=["\']application/json["\'][^>]*>\s*({.*?})\s*</script>', re.DOTALL),
        "json_script_tag",
    ),
]
