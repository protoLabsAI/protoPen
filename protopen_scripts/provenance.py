...
def run_npm_checks(data: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    checks.append({
        ...
        "passed": has_repo,
        "details": str(repo.get('url', repo) if isinstance(repo, dict) else repo) if has_repo else "No repository URL in package metadata",
    })
    ...
    checks.append({
        ...
        "passed": has_author,
        "details": str(author.get('name', author) if isinstance(author, dict) else author) if has_author else "No author in package metadata",
    })
    ...
    if created:
        ...
        checks.append({
            ...
            "passed": not is_fresh,
            "details": f"Package created {age_days} days ago ({created.strftime('%Y-%m-%d')})" + (" — SUSPICIOUSLY RECENT" if is_fresh else ""),
        })
    else:
        checks.append({
            "check_name": "package_age",
        ...
        if release_files:
            ...
            if created:
                ...
                checks.append({
                    "check_name": "package_age",
                    "passed": not is_fresh,
                    "details": f"Latest version uploaded {age_days} days ago" + (" — SUSPICIOUSLY RECENT" if is_fresh else ""),
                })
