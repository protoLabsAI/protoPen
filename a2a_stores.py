...
restart. This module restores the two capabilities the hand-rolled
``a2a_handler.py`` in-memory ``A2ATaskStore`` provided before the SDK migration:

1. **Durable persistence** — SQLite-backed ``DatabaseTaskStore`` /
   ``DatabasePushNotificationConfigStore`` (via SQLAlchemy + aiosqlite), under
   protoPen's data dir (``/sandbox`` → ``~/.protopen`` fallback, matching the
   checkpointer in ``server.py``). The SDK DB stores expose no TTL
...
def _resolve_db_path(leaf: str) -> str:
    ...
    protoPen runs native on the Steam Deck (single instance, no per-instance
    scoping). Mirrors the checkpointer's convention in ``server.py``: prefer
    ``/sandbox/<leaf>``; fall back to ``~/.protopen/<leaf>`` when the sandbox dir
    isn't writable (local dev).
    """
    configured = Path("/sandbox") / leaf
    try:
        ...
        fallback = Path.home() / ".protopen" / leaf
...
async def reconcile_interrupted_tasks(engine: AsyncEngine, *, now: datetime | None = None) -> int:
    ...
    UPDATE mirrors ``sweep_expired_tasks``. Returns the rows reconciled.
    Ported from protoAgent #494/#486.
    """
    now = now or datetime.now(UTC)
...
async def initialize_a2a_stores(
    ...
    Restores the bespoke store's restart behavior the A2A 1.0 migration dropped:
    ...
    terminal ``failed`` rather than being silently deleted at 24h. The
    ``input_required`` / ``auth_required`` pauses are left alone (resumable from
    the checkpoint). Ported from protoAgent #494/#486.
    """
