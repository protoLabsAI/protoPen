"""Tests for the A2A 1.0 port: protolabs_a2a conventions + ProtoPenExecutor.
...
replaced by ``a2a-sdk`` 1.0 + two thin layers:
  ...
  - ``a2a_executor.ProtoPenExecutor`` — bridges protoPen's LangGraph stream
...
from a2a_executor import ProtoPenExecutor, TurnOutcome, set_terminal_hook
...
def _build_app(stream_fn, *, bearer=None, api_key="", allowed_origins=None):
    ...
    handler = DefaultRequestHandler(
        agent_executor=ProtoPenExecutor(stream_fn),
...
async def test_text_deltas_stream_as_incremental_artifact_frames():
    """Token-by-token (#616): each ``text`` delta is forwarded as its own
    artifact-update (append) frame, so the console fills the bubble live instead
    of the whole answer landing at turn end. The terminal closes the same
    ...
    # terminal frame would mean no streaming).
    assert len(artifact_texts) >= 2, artifact_texts
    assert any("First sentence" in t for t in artifact_texts)
    # Concatenated deltas reconstruct the answer exactly once (no double-text from
    # the terminal frame).
    assert "".join(artifact_texts) == full
...
async def test_input_required_form_carries_hitl_datapart():
    ...
    protoPen-local hitl-v1 DataPart carrying the full payload, plus a text
