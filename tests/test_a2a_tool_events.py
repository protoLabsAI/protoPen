...
The producer (`server.py::_chat_langgraph_stream`) yields structured
``("tool_start"|"tool_end", {id, name, input|output})`` tuples; the runner
derives a text status AND a phase-tagged ``tool_event`` stored on
``record.last_tool_event``; ``_build_status_event`` attaches it as a
``tool-call-v1`` DataPart alongside the existing text part.
"""
...
def test_status_event_includes_tool_call_datapart_alongside_text():
    ...
    record.last_status_message = "🔧 dns_enum: example.com"
    ...
    assert text_parts and text_parts[0]["text"] == "🔧 dns_enum: example.com"
...
def test_status_event_terminal_state_drops_tool_event():
    ...
    record.last_status_message = "🔧 dns_enum: ..."
    record.last_tool_event = {"id": "run-1", "name": "dns_enum", "phase": "start"}
...
async def test_update_state_persists_tool_event():
    ...
    evt = {"id": "r1", "name": "lan_scan", "input": "10.0.0.0/24", "phase": "start"}
...
async def test_update_state_clears_tool_event_on_terminal():
    """Terminal transitions wipe the rolling tool event so a completed task
    shows a clean final state, not a stale tool ping."""
    store = A2ATaskStore()
    record = _make_record(state=WORKING)
    record.last_tool_event = {"id": "r1", "name": "lan_scan", "phase": "start"}
...
async def test_runner_records_structured_tool_events():
    ...
    passed to update_state as a phase-tagged tool_event AND a text status is
    derived for back-compat."""
    store = A2ATaskStore()
    await store.create(_make_record(id="bg-tools"))

    # Spy on the tool_event kwarg the runner hands to update_state — push_fn
    # only fires on the initial WORKING + terminal transitions, so it can't
    # observe the mid-run tool pings.
    captured: list[dict] = []
    ...
    store.update_state = _spy  # type: ignore[method-assign]

    stream_fn = lambda: _mock_stream(
        ("text", "working"),
        ("tool_start", {"id": "r1", "name": "dns_enum", "input": "example.com"}),
        ("tool_end", {"id": "r1", "name": "dns_enum", "output": "3 records"}),
        ("done", "working"),
    )
    ...
    with patch("a2a_handler._store", store):
        try:
            await _run_task_background("bg-tools", stream_fn, _push)
        finally:
            store.update_state = orig_update  # type: ignore[method-assign]
    ...
    assert starts and starts[0]["input"] == "example.com"
    assert ends and ends[0]["output"] == "3 records"
...
async def test_runner_accepts_legacy_string_tool_payloads():
    ...
    store = A2ATaskStore()
    await store.create(_make_record(id="bg-legacy"))
    ...
    with patch("a2a_handler._store", store):
        await _run_task_background("bg-legacy", stream_fn, _push)
...
def _make_tool_app():
    ...
    async def _fake_stream(text, context_id):
        ...
        yield ("tool_start", {"id": "r1", "name": "dns_enum", "input": "example.com"})
        await asyncio.sleep(0.03)
        yield ("tool_end", {"id": "r1", "name": "dns_enum", "output": "3 records"})
        await asyncio.sleep(0.03)
        yield ("done", "Found 3 records.")

    async def _fake_chat(text, session_id):
        return [{"role": "assistant", "content": "Found 3 records."}]
    ...
    async with httpx.AsyncClient(
        ...
        async with client.stream(
            ...
            json={
                ...
                "params": {"message": {"parts": [{"kind": "text", "text": "scan example.com"}]}},
            },
        ) as resp:
