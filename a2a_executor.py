"""protoPen's A2A 1.0 AgentExecutor — drives LangGraph through ``a2a-sdk``.
...
protoPen's existing ``_chat_langgraph_stream`` event generator
...
# protoPen-LOCAL extension (not one of the four fleet extensions in
...
class ProtoPenExecutor(AgentExecutor):
    """Bridges protoPen's LangGraph stream onto the A2A event queue.
    ...
    def __init__(
        ...
        # async generator of (event_type, payload). This is protoPen's
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        # Live answer streaming (#616): forward each text delta as an incremental
        ...
        # emission then closes the SAME artifact: if text was streamed it appends
        # ONLY the cost/confidence DataParts (body=""), so concat-based consumers
        # don't double the answer; otherwise (no deltas — e.g. a short-circuit)
        # it emits the full text once, exactly as before.
