...
When the agent is wired with many tools (protoPen binds ~80 native security/
pentest tools), every bound tool's name + description + JSON schema is sent to
the model on *every* turn. Past ~10–15 tools that burns context and degrades
tool selection ("tool pollution"). This middleware withholds most tool *schemas*
from the model while keeping every tool *callable*:
