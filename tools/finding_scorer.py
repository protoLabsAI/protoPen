...
_RULES: list[tuple[str, str, str, str, dict[str, str], str, str]] = [
    ...
    (
        ...
        {"target": "https://{target}"},
        "active",
        "JWT algorithm confusion and claim manipulation",
    ),
    (
        "web",
        "graphql",
        ...
        "active",
        "GraphQL schema extraction and fuzzing",
    ),
    (
        "web",
        "sql",
    ...
    (
        ...
        {"target": "https://{target}"},
        "active",
        "SSRF probe against internal endpoints",
    ),
    # ─ API / Auth ───────────────────────────────────────────────────────────
    ...
    (
        ...
        {"target": "https://{target}/graphql"},
        "active",
        "GraphQL field and query fuzzing",
    ),
    (
        "auth",
        "oauth",
        ...
        {"target": "https://{target}"},
        "active",
        "OAuth/OIDC flow and token testing",
    ),
    (
        "auth",
        "jwt",
        ...
        {"target": "https://{target}"},
        "active",
        "JWT key confusion and brute-force",
    ),
    (
        "auth",
        "bypass",
        ...
        {"target": "https://{target}"},
        "active",
        "Broken object-level auth (BOLA/IDOR) testing",
    ),
    (
        "auth",
        "saml",
        ...
        {"target": "https://{target}"},
        "active",
        "SAML assertion forge and redirect tests",
    ),
    # ─ SSL / TLS ─────────────────────────────────────────────────────────────
    ...
    (
        ...
        {"target": "{target}"},
        "active",
        "Enumerate SMB shares, users, policies",
    ),
    (
        "network",
        "rdp",
    ...
    (
        ...
        {"target": "{target}"},
        "passive",
        "Match discovered services against known CVEs",
    ),
    (
        "service",
        "",
    ...
    (
        ...
        "sql_test",
        "scan",
        {"target": "{target}", "dbms": "mysql"},
        "active",
        "MySQL injection and credential testing",
    ),
    (
        "database",
        "mssql",
        ...
        {"target": "{target}"},
        "active",
        "MSSQL brute-force and xp_cmdshell",
    ),
    (
        "database",
        "redis",
    ...
    (
        ...
        {"hash_type": "22000"},
        "active",
        "Generate hashcat rules for PMKID cracking",
    ),
    (
        "wifi",
        "pmkid",
    ...
    (
        ...
        {"hash_type": "NT"},
        "active",
        "Generate hashcat rules for captured hashes",
    ),
    (
        "credential",
        "",
        ...
        {"target": "{target}"},
        "active",
        "Credential spray against discovered services",
    ),
    # ─ CVE ───────────────────────────────────────────────────────────────────
    ...
    (
        ...
        {"target": "{target}"},
        "active",
        "Subscribe to MQTT broker, extract topics/payloads",
    ),
    (
        "iot",
        "",
        ...
        {"target": "{target}"},
        "active",
        "Discover IoT protocols on target segment",
    ),
    # ─ Active Directory ──────────────────────────────────────────────────────
...
def deduplicate(findings: list[dict]) -> list[dict]:
    ...
    for f in findings:
        ...
        if fid not in seen:
            ...
            if _SEVERITY_RANK.get(f.get("severity", "info"), 0) > _SEVERITY_RANK.get(
                seen[fid].get("severity", "info"), 0
            ):
...
def suggest_followup(finding: dict, mode: str = "active") -> list[AttackSuggestion]:
    ...
    for cat_kw, title_kw, tool, action, tmpl, min_mode, desc in _RULES:
        ...
        resolved = {k: v.replace("{target}", target).replace("{cve_id}", cve_id or "") for k, v in tmpl.items()}
        suggestions.append(
            AttackSuggestion(
                tool=tool,
                action=action,
