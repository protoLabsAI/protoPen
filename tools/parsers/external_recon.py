...
def parse_dns_security(raw: str, store: "TargetStore") -> list[dict]:
    ...
    if "p=none" in raw:
        entities.append({"type": "finding", "severity": "low", "title": "DMARC policy=none (no enforcement)", "detail": raw[:200]})
    if "+all" in raw:
        entities.append({"type": "finding", "severity": "high", "title": "SPF +all — email spoofing trivial", "detail": raw[:200]})
    return entities
